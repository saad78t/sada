...
function RenderComment({
  ...
  // هنا نتحقق إذا السلسلة كلها محذوفة
  const isFullyDeleted = isThreadFullyDeleted(comment, repliesMap);

  // إذا التعليق محذوف وكل ردوده محذوفة أو ماكو ردود، ما نعرضه
  if (isFullyDeleted && nestedReplies.length === 0) {
    return null;
  }
    ...
    <CommentContainer key={comment.id}>
      ...
      <CommentContent>
        ...
        {showReplies && (
          <RepliesWrapper>
            {nestedReplies
              .filter((reply) => !isThreadFullyDeleted(reply, repliesMap)) // فلترة الردود المحذوفة بالكامل
              .map((reply) => (
                <RenderComment
                  key={reply.id}
                  comment={reply}
